...
for b = 1:total_block
    ...
    for i = 1:MicNum
        for j = 1:MicNum
            e_ul(i, j, :) = W_01_Lx2L*(squeeze(D(i, :, :))*W_10_2LxL*h_hat_ul((j-1)*L+1:j*L, :)-squeeze(D(j, :, :))*W_10_2LxL*h_hat_ul((i-1)*L+1:i*L, :));
        end
    end
    ...
    % update S %
    for i = 1:MicNum
        S(i, :, :) = W_01_Lx2L*squeeze(D(i, :, :))*W_10_2LxL;
    end
    ...
    for i = 1:MicNum
        for j = 1:MicNum
            R((i-1)*L+1:i*L, (j-1)*L+1:j*L) = -squeeze(S(j, :, :))'*squeeze(S(i, :, :));
            if i == j
                dia_sum = dia_sum + squeeze(S(j, :, :))'*squeeze(S(i, :, :));
            end
    ...
    for i = 1:MicNum
        R((i-1)*L+1:i*L, (i-1)*L+1:i*L) = R((i-1)*L+1:i*L, (i-1)*L+1:i*L) + dia_sum;
    end
    
    h_hat_ul = (h_hat_ul - step_size*R*h_hat_ul)/norm(h_hat_ul - step_size*R*h_hat_ul)/sqrt(L);
    

end

h_hat = real((inv(F_LxL)*reshape(h_hat_ul, [L MicNum])).');
% rescale h_hat %
