...
for n = points_rir:SorLen
    X = zeros(MicNum-1, MicNum*points_rir);
    for i = 1:MicNum-1
        X(i, 1:points_rir) = flip(x(i+1, n-points_rir+1:n));
        X(i, i*points_rir+1:(i+1)*points_rir) = -flip(x(1, n-points_rir+1:n));
    end

    Rxx = (1-forfact)*Rxx + forfact*(X.')*X;
end
