...
%% construct Rxx (Rxx) %%
forfact = 2^(-9);
Rxx = zeros(frequency, MicNum*L, MicNum*L);
parfor n = 1:frequency
    for FrameNo = L:NumOfFrame
        X_matrix = zeros(MicNum-1, MicNum*L);
        for i = 1:MicNum-1
            X_matrix(i, 1:L) = flip(squeeze(X(n, FrameNo-L+1:FrameNo, i+1)));
            X_matrix(i, i*L+1:(i+1)*L) = flip(squeeze(-X(n, FrameNo-L+1:FrameNo, 1)));
        end
...
eigen_vector = zeros(frequency, MicNum*L, MicNum*L);
eigen_value = zeros(frequency, MicNum*L);
...
for n = 1:frequency
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    H_hat(:, :, n) = reshape(squeeze(eigen_vector(n, :, min_index(n, :))), [L MicNum]).';
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
end
...
plot(h(look_mic, :), 'r');
hold on
plot(h_hat(look_mic, hopsize*(osfac-1)+1:end), 'b');
hold off
xlim([1 points_rir])
...
figure(3)
plot(1:1:MicNum*L, eigen_value);
title('eigenvalue')
ylabel('linear scale')

figure(4)
plot(1:1:MicNum*L, log(eigen_value));
title('eigenvalue')
ylabel('log scale')
