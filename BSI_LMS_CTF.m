...
for n = points_rir:SorLen
    % construct R %
    R_temp = zeros(MicNum*points_rir, MicNum*points_rir);
    ...
    for i = 1:MicNum
        R_temp((i-1)*points_rir+1:i*points_rir, (i-1)*points_rir+1:i*points_rir) = R_temp((i-1)*points_rir+1:i*points_rir, (i-1)*points_rir+1:i*points_rir) + dia_sum;
    end

    % construct cost function %
    for i = 1:MicNum-1
        for j = i+1:MicNum
            cost_fun(n, :) = cost_fun(n, :) + (flip(x(i, n-points_rir+1:n))*h_hat((j-1)*points_rir+1:j*points_rir, :) - flip(x(j, n-points_rir+1:n))*h_hat((i-1)*points_rir+1:i*points_rir, :))^2;
        end

    end

    % compute new h_hat %
    h_hat = h_hat-2*step_size*(R_temp*h_hat-cost_fun(n, :)*h_hat)/norm(h_hat-2*step_size*(R_temp*h_hat-cost_fun(n, :)*h_hat));
